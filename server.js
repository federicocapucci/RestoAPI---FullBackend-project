...
app.post("/registro", async (req, res) => {
  ...
  try {
    ...
    db.query(
      "INSERT INTO Usuarios (Nombre, Apellido, Email, telefono, Direccion, Password)  VALUES (?,?,?,?,?,?)",
      {
        replacements: [
          Nombre,
          Apellido,
    ...
    .then(async (respuesta) => {
      ...
      if (resultado.length != 0) {
        ...
        if (hashCheck || resultado[0].Email == "admin@root.com") {
          ...
          ); //Creamos un token para el usuario que expira en 'x' tiempo
...
app.put("/usuario", tokenVerifier, (req, res) => {
  ...
  let { Nombre, Apellido, Email, Telefono, Direccion, Password } = req.body;
  ...
  db.query("select * from usuarios where usuario_ID = ?", {
    ...
    .then(async (respuesta) => {
      if (respuesta[0].length != 0) {
        ...
        db.query(
          "UPDATE usuarios SET Nombre = ? , Apellido = ? , Email = ?, Telefono = ?, Direccion = ?, Password = ? where usuario_ID = ?",
          {
            replacements: [
              Nombre,
              Apellido,
          ...
          .catch((err) =>
            res.status(409).send({ msg: "Este email ya esta registrado" })
          );
      } else {
...
app.post("/pedidos", tokenVerifier, (req, res) => {
  ...
  db.query(
    "insert into pedidos (usuario_ID,TipoPago,Estado,Fecha,Total) values(?,?,?,?,?)",
    {
      replacements: [
        req.token.data[0],
        pedido.tipoPago,
      ...
      if (respuesta[0].length != 0) {
        db.query("UPDATE pedidos SET Estado = ? WHERE Pedido_ID = ?", {
          ...
          .then((respuesta) => {
            res.status(200).send({
              msg:
                "El estado del pedido ha sido modificado a " + req.body.status,
            });
          })
          .catch((err) =>
            res.status(500).send({ msg: "ha ocurrido un error", err: err })
          );
      } else {
  ...
  if (req.isAdmin) {
    ...
    db.query("SELECT * FROM pedidos where pedido_ID = ?", {
      ...
      if (respuesta[0].length != 0) {
        ...
        res.status(200).send({
          msg: "El pedido " + pedido_ID + "fue removido exitosamente",
        });
      } else {
        return res
          .status(404)
          .send({
            msg:
              "El numero de pedido " + pedido_ID + " fue borrado o no existe",
          });
      }
    });
  } else {
  ...
  if (req.isAdmin) {
    ...
    db.query("insert into productos (Nombre,Precio,Foto) values(?,?,?)", {
      ...
      .then((respuesta) => {
        res
          .status(201)
          .send({ msg: "Producto '" + Nombre + "' creado correctamente" });
      })
      .catch((err) => res.status(500).send(err));
  } else {
    res.status(401).send({ msg: "No estas autorizado" });
...
app.put("/platos", tokenVerifier, (req, res) => {
  if (req.isAdmin) {
    ...
    let { Nombre, Precio, Foto } = req.body.datos;
    db.query("select * from productos where producto_ID = ?", {
      ...
      .then((respuesta) => {
        if (respuesta[0] != 0) {
          ...
          db.query(
            "UPDATE productos SET Nombre = ? , Precio = ? , Foto = ? where producto_ID = ?",
            {
          ...
          res
            .status(404)
            .send({ msg: "El plato no se encuentra en la base de datos" });
        }
      })
      .catch((err) => res.status(500).send(err));
  } else {
  ...
  if (req.isAdmin) {
    ...
    db.query("SELECT * from productos where Producto_ID = ?", {
      ...
      if (respuesta[0].length != 0) {
        ...
        return res
          .status(200)
          .send({ msg: "El plato fue removido exitosamente" });
      } else {
